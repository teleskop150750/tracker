<template>
    <div class="v-select">
        <label v-if="label" class="v-select__label" :for="`${uid}__input`">
            {{ label }}
        </label>
        <div ref="containerRef" :id="uid" class="v-select__select" :class="containerClass" @click="onContainerClick">
            <div class="v-select__select-header">
                <input
                    ref="inputRef"
                    :id="`${uid}__input`"
                    @focus="onInputFocus"
                    ...
                    v-bind="inputAttrs"
                />
                <div class="v-select__select-header-inner">
                        ...
                        <slot name="value" :value="modelValue" :placeholder="placeholder">
                            <template v-if="multiple">
                                ...
                                <template v-else-if="display === 'chip'">
                                    <VChip
                                        class="v-select__select-label-chip"
                            ...
                            <template v-else>
                                ...
                                <template v-else-if="display === 'chip'">
                                    <VChip
                                        class="v-select__select-label-chip"
                                        :key="getLabelByValue(modelValue)"
                                        @delete="removeOption(modelValue)"
                                    >
                                        <slot name="chip" :value="modelValue">
                                            <span>{{ getLabelByValue(modelValue) }}</span>
                                        </slot>
                                    </VChip>
                                    <template v-if="!modelValue || modelValue.length === 0">
                                        {{ placeholder || 'empty' }}
                ...
                <div class="v-select__select-trigger">
                    <slot name="indicator">
                        <VArrowDownIcon class="v-select__select-trigger-icon" aria-hidden="true" />
                    </slot>
                </div>
            </div>
                ...
                >
                    ...
                    >
                        ...
                        <div
                            ...
                            class="v-select__select-panel-header"
                            :class="[
                                isToggleAllShown && 'v-select__select-panel-header--with-select-all',
                                filter && 'v-select__select-panel-header--with-filter',
                            ]"
                        >
                            <VCheckbox
                            ...
                            />
                            <div v-if="filter" class="v-select__select-filter-container">
                                <input
                                    type="text"
                                    class="v-select__select-filter"
                                    :value="filterValue"
                        ...
                        <div
                            class="v-select__select-panel-items-wrapper"
                            ...
                            >
                                <template v-slot:content="{ styleClass, contentRef, items, getItemOptions, contentStyle, itemSize }">
                                    <ul
                                        ...
                                        :id="`${uid}_list`"
                                        :class="['v-select__select-items', styleClass]"
                                        :style="contentStyle"
                                        role="listbox"
                                    ...
                                    >
                                        ...
                                        >
                                            <li
                                                ...
                                                class="v-select__select-item-group"
                                                role="option"
                                            >
                                                <slot
                                                    name="option-group"
                                            ...
                                            <li
                                                ...
                                                :style="{ height: itemSize ? itemSize + 'px' : undefined }"
                                                :class="[
                                                    'v-select__select-item',
                                                    {
                                                        'v-select__select-item--with-checkbox': multiple,
                                                        'v-select__select-item--selected': isSelected(option) && !props.multiple,
                                                        'v-select__select--focus': focusedOptionIndex === getOptionIndex(i, getItemOptions),
                                                        'v-select__select--disabled': isOptionDisabled(option),
                                                    },
                                                ]"
                                                role="option"
                                        ...
                                        <li
                                            ...
                                            class="v-select__select-empty-message"
                                            role="option"
                                        >
...
import { getCurrentInstance, ref, computed, useSlots, onUpdated, onBeforeUnmount } from 'vue'
...
import { useSelectFocus } from './useSelectFocus.js'
import { useSelectFilter } from './useSelectFilter.js'
...
const { updateModelValue } = useSelect(props, emit)
const {
    equalityKey,
    selectedMessage,
    isSelected,
    ...
    getOptionGroupLabel,
    useVisibleOptions,
} = useSelectOptions(props, emit)
const { searchValue, searchFields } = useSelectSearch(props)
const { filterValue, getFilterResultMessage } = useSelectFilter(props, emit)
const { focused, focusedOptionIndex, focusedOptionId } = useSelectFocus(props, emit)
const {
    flatOptions,
    filteredOptions,
    allSelected,
...
const containerClass = computed(() => [
    {
        'v-select__select--disabled': props.disabled,
        'v-select__select--clearable': props.showClear && !props.disabled,
        'v-select__select--focus': focused.value,
        'v-select__select--open': overlayVisible.value,
    },
])
const labelClasses = computed(() => [
    'v-select__select-label',
    props.inputClass,
    {
        'v-select__select-label-placeholder': valueLabel.value === props.placeholder,
        'v-select__select-label-empty': !slots['value'] && (!valueLabel.value || valueLabel.value.length === 0),
    },
])
...
const inputAttrs = computed(() => {
    ...
    let attrs = {
        ...{
            ...
            ariaLabel: props.ariaLabel || valueLabel.value,
...
function removeOption(optionValue) {
    ...
    if (props.multiple) {
        value = props.modelValue.filter((val) => !ObjectUtils.equals(val, optionValue, equalityKey.value))
    }
...
.v-select__select:not(.v-select__select--disabled):hover {
    border-color: hsl(210deg 14% 83%);
}

.v-select__select--focus:not(.v-select__select--disabled) {
...
.v-select__select-label:focus,
.v-select__select-label:enabled:focus {
    outline: 0 none;

    box-shadow: none;
}

.v-select__select-label-placeholder {
    color: hsl(208deg 7% 46%);
}

.v-select__select-label-empty {
    opacity: 0;

    overflow: hidden;
}

input.v-select__select-label {
    cursor: default;
}

.v-select__select-label-chip + .v-select__select-label-chip {
    margin-left: 0.5rem;
}

.v-select__select-panel {
    position: absolute;
    top: 0;
    left: 0;
...
.v-select__select-panel-header--with-select-all {
    display: grid;
...
.v-select__select-panel-header--with-filter {
    padding: 0.5rem 1.5rem 0.5rem 0.5rem;
}

.v-select__select-panel-items-wrapper {
    overflow: auto;
}

.v-select__select-filter-container {
    position: relative;

    width: 100%;
}

.v-select__select-filter {
    display: block;
...
.v-select__select-filter::placeholder {
    color: hsl(214deg 20% 69%);
}

.v-select__select-filter:focus {
    border-color: rgb(172 148 250);
...
.v-select__select-item--selected {
    color: hsl(0deg 0% 100%);

    background: hsl(262deg 88% 59%);
}

.v-select__select-item--selected.v-select__select--focus {
    background: hsl(262deg 88% 52%);
}

.v-select__select-item:not(.v-select__select-item--selected, .v-select__select--disabled).v-select__select--focus {
    color: hsl(210deg 11% 15%);

    background: hsl(210deg 14% 89%);
}

.v-select__select-item:not(.v-select__select-item--selected, .v-select__select--disabled):hover {
    color: hsl(210deg 11% 15%);

    background: hsl(210deg 16% 93%);
}

.v-select__select-item--with-checkbox {
    display: flex;
    align-items: center;
...
.v-select__select.v-invalid {
    border-color: hsl(354deg 70% 54%);
}

.v-select__select-empty-message {
    padding: 0.5rem 1.5rem;
