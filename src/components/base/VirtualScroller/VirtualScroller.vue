<template>
    <template v-if="!disabled">
        <div :ref="elementRef" class="v-virtual-scroller" :class="containerClass" :tabindex="tabindex" :style="style" @scroll="onScroll">
            <slot
                name="content"
            ...
            >
                <div :ref="contentRef" :class="contentClass" :style="contentStyle">
                    <template v-for="(item, index) of loadedItems" :key="index">
                        <slot name="item" :item="item" :options="getOptions(index)" />
                    </template>
                </div>
            </slot>
            <div class="v-virtual-scroller-spacer" :style="spacerStyle" v-if="showSpacer"></div>
            <div class="v-virtual-scroller-loader" :class="loaderClass" v-if="!loaderDisabled && showLoader && d_loading">
                <template v-if="$slots && $slots.loader">
                    <template v-for="(_, index) of loaderArr" :key="index">
                        <slot name="loader" :options="getLoaderOptions(index, isBoth && { numCols: d_numItemsInViewport.cols })" />
                    </template>
                </template>
                <VProgressSpinner v-else style="width: 30px" />
            </div>
        </div>
    </template>
    ...
    <template v-else>
        <slot />
        <slot name="content" :items="items" :rows="items" :columns="loadedColumns" />
    </template>
</template>

<script>
import VProgressSpinner from '../VProgressSpinner/VProgressSpinner'
...
export default {
    ...
    data() {
        return {
            first: this.orientation === 'both' ? { rows: 0, cols: 0 } : 0,
            last: this.orientation === 'both' ? { rows: 0, cols: 0 } : 0,
            numItemsInViewport: this.orientation === 'both' ? { rows: 0, cols: 0 } : 0,
            lastScrollPos: this.orientation === 'both' ? { top: 0, left: 0 } : 0,
    ...
    mounted() {
        ...
        this.lastScrollPos = this.orientation === 'both' ? { top: 0, left: 0 } : 0
    },
    watch: {
    ...
    methods: {
        ...
        scrollToIndex(index, behavior = 'auto') {
            ...
            if (isBoth) {
                ...
                if (newFirst !== first) {
                    isHorizontal ? scrollTo(calculateCoord(newFirst, itemSize), 0) : scrollTo(0, calculateCoord(newFirst, itemSize))
                }
            }
        },
        ...
        onScrollPositionChange(event) {
            ...
            let newFirst = isBoth ? { rows: 0, cols: 0 } : 0
    ...
    computed: {
        ...
        containerClass() {
            return [
                {
                    'v-both-scroll': this.isBoth,
                    'v-horizontal-scroll': this.isHorizontal,
                },
                this.class,
            ]
        },
        ...
        contentClass() {
            return [
                'v-virtual-scroller-content',
                {
                    'v-virtual-scroller-loading': this.d_loading,
                },
            ]
        },
        loaderClass() {
            return { 'v-component-overlay': !this.$slots.loader }
        },
        loadedItems() {
            const items = this.items
                ...
                if (isBoth || isHorizontal) {
                    return this.d_loading && this.loaderDisabled
                        ? isBoth
                            ? this.loaderArr[0]
                            : this.loaderArr
                        : this.columns.slice(isBoth ? this.first.cols : this.first, isBoth ? this.last.cols : this.last)
                }
            }
...
.v-virtual-scroller .v-virtual-scroller-loader {
    position: sticky;
    top: 0;
    left: 0;
...
.v-virtual-scroller-loader.v-component-overlay {
    display: flex;
    justify-content: center;
    align-items: center;

    background-color: hsl(0deg 0% 7% / 40%);
}
</style>
